...
def CreatReadme(ruleList, fileName):
    ...
    with open(fileName, 'a') as f:
        ...
        f.write("适用于AdGuard的去广告合并规则，每天更新一次。\n")
        ...
        f.write("## 上游规则源\n")
        f.write("感谢以下广告过滤规则维护大佬们的辛苦付出。\n")
        f.write("\n")
        f.write("| 规则 | 类型 | 原始链接 | 加速链接 | 更新日期 |\n")
        f.write("|:-|:-|:-|:-|:-|\n")
        for rule in ruleList:
            f.write("| %s | %s | [原始链接](%s) | [加速链接](https://quantil.jsdelivr.net/gh/YujiaCheng1996/adblockfilters@main/rules/%s.txt) | %s |\n" % (rule[0],rule[1],rule[2],rule[0].replace(' ', '_'),rule[3]))
...
def CreatDNS(blockDict, unblockDict, fileName):
    ...
    with open(fileName, 'a') as f:
        ...
        f.write("! Description: 适用于AdGuard的去广告合并规则，每天更新一次，规则源详见主页。\n")
        f.write("! Homepage: https://github.com/YujiaCheng1996/adblockfilters\n")
        f.write("! Source: https://raw.githubusercontent.com/YujiaCheng1996/adblockfilters/main/rules/adblockdns.txt\n")
...
def CreatFiter(filterList, fileName):
    ...
    with open(fileName, 'a') as f:
        ...
        f.write("! Description: 适用于AdGuard的去广告合并规则，每天更新一次，规则源详见主页。\n")
        f.write("! Homepage: https://github.com/YujiaCheng1996/adblockfilters\n")
        f.write("! Source: https://raw.githubusercontent.com/YujiaCheng1996/adblockfilters/main/rules/adblockfilters.txt\n")
        f.write("! Version: %s\n"%(time.strftime("%Y%m%d%H%M%S", time.localtime())))
        f.write("! Last modified: %s\n"%(time.strftime("%Y/%m/%d %H:%M:%S", time.localtime())))
